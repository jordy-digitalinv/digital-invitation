...
export default function AmbonManiseTemplate({ guest, client, token }: TemplateProps) {
  ...
  return (
    <div style={{ background: bg, color: text, fontFamily: `'${fontB}', sans-serif`, minHeight: "100dvh" }}>
      <style>{`
        ...
        .am-cengkeh { display:inline-block; width:.85rem; height:.85rem; background:${laut};
          clip-path:polygon(50% 0%, 62% 38%, 100% 50%, 62% 62%, 50% 100%, 38% 62%, 0% 50%, 38% 38%); }
        .am-orn { display:flex; align-items:center; gap:.7rem; justify-content:center; }
        .am-orn::before,.am-orn::after { content:""; height:1px; flex:0 0 40px; background:linear-gradient(90deg,transparent,${laut}77); }
        .am-orn::after { background:linear-gradient(90deg,${laut}77,transparent); }
        .am-perahu::after { content:"⛵"; }
      `}</style>
      ...
      {!open && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center text-center px-8" style={{ background: surface }}>
          ...
          <Waves size={26} style={{ color: laut }} className="relative z-10 mb-5 animate-pulse" />
          ...
          <p className="relative z-10 mt-4 text-[11px] italic" style={{ color: DEF.muted }}>— dari Ambon yang manise —</p>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
                  ...
                  <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
                    ...
                    {p.photo && (
                      ...
                      <img src={p.photo} alt={p.name} className="w-44 h-52 object-cover mx-auto mb-5"
                        style={{ borderRadius: "50% 50% 42% 42% / 46% 46% 54% 54%", border: `3px solid ${surface}`, boxShadow: `0 10px 26px ${DEF.muted}33` }} />
                    )}
                ...
                {visibleEvents.map((ev) => (
                  <motion.div key={ev.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                    ...
                    {(ev.timeStart || ev.timeEnd) && (
                      <p className="text-sm flex items-center justify-center gap-2"><Clock size={13} style={{ color: laut }} />{ev.timeStart}{ev.timeEnd ? ` - ${ev.timeEnd}` : ""} WIT</p>
                    )}
                    <div>
          ...
          <footer className="py-20 px-7 text-center am-wave-top" style={{ background: surface }}>
            ...
            <p className="mt-5 text-xs max-w-xs mx-auto leading-relaxed" style={{ color: DEF.muted }}>
              Beta sekeluarga tunggu kehadiran Bapak/Ibu/Saudara/i — sampe jumpa di Ambon manise!
            </p>
  ...
  if (done) {
    return (
      <div className="max-w-sm mx-auto text-center bg-white/80 rounded-2xl px-6 py-9">
        ...
        <p className="mt-3 text-sm">Beta tunggu konfirmasi lainnya — makasih!</p>
      </div>
    );
  }
