...
export default function SangjitMerahTemplate({ guest, client, token }: TemplateProps) {
  ...
  return (
    <div style={{ background: bg, color: text, fontFamily: `'${fontB}', sans-serif`, minHeight: "100dvh" }}>
      <style>{`
        ...
        .am-orn::before,.am-orn::after { content:""; height:2px; flex:0 0 40px; background:linear-gradient(90deg,transparent,#f5c542aa); }
        .am-orn::after { background:linear-gradient(90deg,#f5c542aa,transparent); }
        .sg-lampion { width:1.4rem; height:1.7rem; border-radius:50%/45%; display:inline-block;
          background:radial-gradient(circle at 35% 30%, #e74c3c, ${laut}); box-shadow:inset -3px -4px 6px #00000033; }
      `}</style>
      ...
      {!open && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center text-center px-8" style={{ background: surface }}>
          ...
          <span className="sg-lampion relative z-10 mb-5" />
          ...
          <p className="relative z-10 mt-4 text-[11px] italic" style={{ color: DEF.muted }}>Gong Xi — Double Happiness</p>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
          ...
          <section className="relative flex flex-col items-center justify-end text-center overflow-hidden" style={{ minHeight: "88dvh", padding: "3rem 1.75rem 5rem" }}>
            ...
            <div className="relative z-10">
              <p className="text-3xl mb-2" style={{ color: "#f5c542", fontFamily: "serif" }}>{XI}</p>
              <p className="text-[10px] uppercase" style={{ letterSpacing: "0.35em", color: laut }}>{invLabel}</p>
                  ...
                  <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
                    ...
                    {p.photo && (
                      ...
                      <img src={p.photo} alt={p.name} className="w-44 h-52 object-cover mx-auto mb-5"
                        style={{ borderRadius: "18px", border: `3px solid ${surface}`, boxShadow: `0 10px 26px ${DEF.muted}33` }} />
                    )}
                    ...
                    <div className="am-orn my-3"><span style={{ color: "#f5c542", fontSize: ".8rem" }}>{XI}</span></div>
          ...
          <footer className="py-20 px-7 text-center am-wave-top" style={{ background: surface }}>
            <span style={{ color: "#f5c542", fontSize: "1rem" }}>{XI}</span>
            ...
            <p className="mt-5 text-xs max-w-xs mx-auto leading-relaxed" style={{ color: DEF.muted }}>
              Kehadiran Bapak/Ibu/Saudara/i merupakan kehormatan dan kebahagiaan besar bagi kami.
            </p>
...
function Head({ laut, fontH, eyebrow, title }: { laut: string; fontH: string; eyebrow: string; title?: string }) {
  return (
    <div className="text-center mb-10">
      ...
      <div className="am-orn mt-4"><span style={{ color: "#f5c542", fontSize: ".8rem" }}>{XI}</span></div>
    </div>
  );
}
...
function RsvpForm({
  clientId, guest, token, laut, needsSoup, onConfirmed,
}: {
  clientId: string; guest: Guest; token: string; laut: string; needsSoup: boolean;
  ...
  if (done) {
    return (
      <div className="max-w-sm mx-auto text-center bg-white/80 rounded-2xl px-6 py-9">
        ...
        <p className="mt-3 text-sm">Terima kasih atas konfirmasinya — Gong Xi Fa Cai!</p>
      </div>
    );
  }
