...
export default function MinangGadangTemplate({ guest, client, token }: TemplateProps) {
  ...
  return (
    <div style={{ background: bg, color: text, fontFamily: `'${fontB}', sans-serif`, minHeight: "100dvh" }}>
      <style>{`
        ...
        .am-orn::before,.am-orn::after { content:""; height:1px; flex:0 0 40px; background:linear-gradient(90deg,transparent,#e3b23caa); }
        .am-orn::after { background:linear-gradient(90deg,#e3b23caa,transparent); }
        .mg-songket { background-image:repeating-linear-gradient(45deg, #e3b23c22 0 2px, transparent 2px 10px), repeating-linear-gradient(-45deg, #8c2f3918 0 2px, transparent 2px 10px); }
      `}</style>
      ...
      {!open && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center text-center px-8" style={{ background: surface }}>
          ...
          <p className="relative z-10 mt-4 text-[11px] italic" style={{ color: DEF.muted }}>— Baralek Gadang, Urang Awak —</p>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
                  ...
                  <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
                    ...
                    {p.photo && (
                      ...
                      <img src={p.photo} alt={p.name} className="w-44 h-52 object-cover mx-auto mb-5"
                        style={{ borderRadius: "50% 50% 8px 8px", border: `3px solid ${surface}`, boxShadow: `0 10px 26px ${DEF.muted}33` }} />
                    )}
          ...
          <footer className="py-20 px-7 text-center am-wave-top" style={{ background: surface }}>
            ...
            <p className="mt-5 text-xs max-w-xs mx-auto leading-relaxed" style={{ color: DEF.muted }}>
              Kehadiran Bapak/Ibu/Saudara/i baralek gadang kami merupakan suatu kehormatan besar bagi urang banyak.
            </p>
  ...
  if (done) {
    return (
      <div className="max-w-sm mx-auto text-center bg-white/80 rounded-2xl px-6 py-9">
        ...
        <p className="mt-3 text-sm">Tarimo kasih atas konfirmasinya!</p>
      </div>
    );
  }
