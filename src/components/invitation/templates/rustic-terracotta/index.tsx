      ...
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
          ...
          {has("WISHES") && (
            ...
            <WishesLockedPlaceholder primaryColor={terra} text={text} fontHeading={fontH} lang="id" />
          ) : (
...
function RsvpForm({
  clientId, guest, token, terra, menuItems, onConfirmed,
}: {
  clientId: string; guest: Guest; token: string; terra: string; menuItems: { id: string; name: string }[];
