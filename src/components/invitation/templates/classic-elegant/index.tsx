...
function RsvpForm({
  clientId, guest, token, gold, surface, text, fontB, needsSoup, onConfirmed,
}: {
  clientId: string; guest: Guest; token: string; gold: string; surface: string; text: string; fontB: string;
  needsSoup: boolean; onConfirmed: (s: string) => void;
}) {
  ...
  async function submit() {
    ...
    await fetch("/api/rsvp", {
      ...
      body: JSON.stringify({
        ...
        soupChoices: needsSoup && status === "HADIR" ? soups.slice(0, pax) : undefined,
      }),
    });
    setDone(true);
    onConfirmed(status);
