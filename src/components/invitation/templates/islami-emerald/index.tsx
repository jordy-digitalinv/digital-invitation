...
export default function IslamiEmeraldTemplate({ guest, client, token }: TemplateProps) {
    ...
    <div style={{ background: bg, color: text, fontFamily: `'${fontB}', sans-serif`, minHeight: "100dvh" }}>
      <style>{`
        ...
        .am-cengkeh { display:inline-block; width:.85rem; height:.85rem; background:${laut};
          clip-path:polygon(50% 0%, 63% 37%, 50% 50%, 37% 37%, 0% 50%, 37% 63%, 50% 100%, 63% 63%, 100% 50%, 63% 37%); }
      ...
      {!open && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center text-center px-8" style={{ background: surface }}>
          ...
          <span className="am-cengkeh relative z-10 mb-5" style={{ width:"1rem",height:"1rem" }} />
          ...
          <p className="relative z-10 mt-4 text-xs" style={{ color: laut }}>Assalamu&apos;alaikum Warahmatullahi Wabarakatuh</p>
                  ...
                  <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
                    ...
                    {p.photo && (
                      ...
                      <img src={p.photo} alt={p.name} className="w-44 h-52 object-cover mx-auto mb-5"
                        style={{ borderRadius: "999px 999px 16px 16px", border: `3px solid ${surface}`, boxShadow: `0 10px 26px ${DEF.muted}33` }} />
                    )}
  ...
  if (done) {
    return (
      <div className="max-w-sm mx-auto text-center bg-white/80 rounded-2xl px-6 py-9">
        ...
        <p className="mt-3 text-sm">Jazakumullahu khairan — terima kasih atas konfirmasinya.</p>
      </div>
    );
  }
